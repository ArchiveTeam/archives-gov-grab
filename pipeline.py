...
LUA_SHA1 = get_hash(os.path.join(CWD, 'archives-gov.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            "--warc-header", "archives-gov-dld-script-version: " + VERSION,
            "--warc-header", ItemInterpolation("archives-gov-item: %(item_name)s"),
        ]
        ...
        if item_type == 'ids':
            ...
            for i in range(int(start), int(stop)+1):
                wget_args.extend(['--warc-header', 'archives-gov-id: {i}'.format(**locals())])
                wget_args.append('https://catalog.archives.gov/id/{i}'.format(**locals()))
                wget_args.append('https://research.archives.gov/id/{i}'.format(**locals()))
                wget_args.append('https://research.archives.gov/description/{i}'.format(**locals()))
                wget_args.append('https://catalog.archives.gov/OpaAPI/iapi/v1/id/{i}'.format(**locals()))
                wget_args.append('https://catalog.archives.gov/OpaAPI/iapi/v1/id/{i}/tags'.format(**locals()))
                wget_args.append('https://catalog.archives.gov/OpaAPI/iapi/v1/id/{i}/comments'.format(**locals()))
                wget_args.append('https://catalog.archives.gov/OpaAPI/iapi/v1/id/{i}?searchTerm='.format(**locals()))
                wget_args.append('https://catalog.archives.gov/OpaAPI/iapi/v1/id/{i}?searchTerm=*:*'.format(**locals()))
        else:
